...
func parse_assign(left *token.Tokens, right *token.Tokens, scope *types.Scope) (types.Expression, error) {
	if *options.Trace {
		fmt.Printf("parse_assign\n\t%v\n\t%v\n---\n", left, right)
	}
	...
	return expression.Assignment{left_expression.Location(), left_expression, right_expression}, nil
}
